...
CHYDataModuleBase::CHYDataModuleBase()
{
	m_bEnableNotify = TRUE;
}

int CHYDataModuleBase::AddObserver(HWND hWnd)
{
	int nRet(-1);
...
bool CHYDataModuleBase::RemoveObserver(HWND hWnd)
{
	bool bRet(FALSE);
	vector<HWND>::iterator itr = find(m_arData.begin(), m_arData.end(), hWnd);
	if(itr != m_arData.end())
...
int CHYDataModuleBase::GetObserverCount()
{
	return m_arData.size();
}

HWND CHYDataModuleBase::GetObserverByIndex(INT nIndex)
{
	HWND hRet(NULL);
...
void CHYDataModuleBase::NotifyObservers(WPARAM wParam, LPARAM lParam, UINT nMsg /* = WM_TS_UI_DATA_CHANGED */, BOOL bAsyn /* = FALSE */)
{
	if(m_bEnableNotify)
	{
